...
Collatz(GEN n)
{
  if (typ(n) != t_INT) pari_err_TYPE("Collatz", n);
  if (signe(n) < 1) pari_err_DOMAIN("Collatz", "n", "<", gen_1, n);
  pari_sp ltop = avma;
  ...
  { /* A006884(48), peak value is 34419078320774113520 > ULONG_MAX = 2^64 - 1 */
#else
  while (cmpiu(n, 159487UL) >= 0)
  { /* A006884(48), peak value is 8601188876 > ULONG_MAX = 2^32 - 1 */
#endif
    iterations++;
...
isfactorial(GEN n)
{
  if (typ(n) != t_INT) pari_err_TYPE("isfactorial", n);
  if (signe(n) < 1) return 0;
  if (mod2(n)) return isint1(n);
  ...
  if (nn)
  {
    return
      ...
      nn < 6227020800
        ? (
#endif
            nn < 40320 ? nn == 2 || nn == 6 || nn == 24 || nn == 120 ||
                           nn == 720 || nn == 5040
                       : nn == 40320 || nn == 362880 || nn == 3628800 ||
                           nn == 39916800 || nn == 479001600
#ifdef LONG_IS_64BIT
            )
        : nn < 355687428096000
            ? nn == 6227020800 || nn == 87178291200 || nn == 1307674368000 ||
                nn == 20922789888000
            : nn == 355687428096000 || nn == 6402373705728000 ||
                nn == 121645100408832000 || nn == 2432902008176640000
#endif
      ;
  }
  ...
  {
    ulong p = lpfu(
      mx); // Doesn't really need to be the smallest, just some prime factor
    avma = ltop;
    t = Z_lval(n, p);
...
Faulhaber(long e, GEN a)
{
  ...
  for (; i <= e; ++i)
  {
    ...
    ret = gadd(
      ret, gmul(gmul(binomialuu(e + 1, i), bernfrac(i)), gpowgs(x, e + 1 - i)));
...
cuberootint(GEN x)
{
  ...
  {
    ulong n = itou_or_0(x);
    if (n) return utoipos(cuberoot(n));
...
#define CUTOFF 1627UL
  ...
  // 100000	72.51
  if (last > 300000 ||
      (ulong)last > maxprime()) // TODO: Find good breakover point here
  {
...
ucountPowerfulu(ulong n)
{
  ...
  /*
  pari_printf("Counting squarefrees until %d, values ranging from %lu to
  %lu;\n", crossover, breakpoint, cuberoot(n)); pari_printf("Counting directly
  to %lu.\n", breakpoint);
  */
  ...
  for (k = 1; k <= breakpoint; k++)
    if (issquarefree_small(k)) res += usqrt(n / k) / k;
  return res;
}
...
#define uCBRTis(n, k) itou(cuberootint(divii(n, sqrs(k))))
  ...
  pari_printf("Counting squarefrees until %d, values ranging from %lu to %lu, ",
              crossover, breakpoint, itou(cuberootint(n)));
  ...
  res -= crossover * ucountSquarefree(breakpoint);
  GEN est = divrr(gzeta(gdiv(utoipos(3), gen_2), MEDDEFAULTPREC),
                  szeta(3, MEDDEFAULTPREC));
  est = gmul(est, sqrtr(itor(n, MEDDEFAULTPREC)));
  GEN correction = divrr(gzeta(gdiv(gen_2, utoipos(3)), MEDDEFAULTPREC),
                         szeta(2, MEDDEFAULTPREC));
  ...
  err_printf(
    "Squarefree time:   %6ld ms; partial sum = %lu (%.2f%% of estimated total)\n",
    timer_delay(&T), res, 100.0 * res / rtodbl(est));
  // zeta(3/2)/zeta(3)*sqrt(n)+zeta(2/3)/zeta(2)*n^(1/3)
  timer_start(&T);
  if (breakpoint > LONG_MAX)
    pari_err_OVERFLOW(
      "Breakpoint much too large, something funny is going on.");
  for (k = 1; k <= breakpoint; k++)
  {
...
countPowerful(GEN n)
{
  ...
  if (sz <= 4)
  {
    ...
    else if (1) //(cmpii(n, mkintn(4, 909366712, 745454542, 1225794890,
                // 2766209793)) < 0)
#else
  ...
  {
    if (Z_issquarefree(k)) ret = addii(ret, sqrti(gdivent(n, powis(k, 3))));
    if (low_stack(st_lim, stack_lim(btop, 1))) gerepileall(btop, 2, &ret, &k);
  }
  ret = gerepileupto(ltop, ret);
  return ret;
}
...
countSquarefree(GEN lim)
{
  ...
  for (k = addis(b, 1); cmpii(k, p3) <= 0; k = addis(k, 1))
  {
    ...
    if (low_stack(st_lim, stack_lim(btop, 1))) gerepileall(btop, 2, &p4, &k);
  }
  ret = addii(ret, p4);
  ret = gerepileupto(ltop, ret);
  return ret;
...
solvePell(GEN n)
{
  if (typ(n) != t_INT) pari_err_TYPE("solvePell", n);
  if (signe(n) < 1) pari_err_DOMAIN("solvePell", "n", "<=", gen_0, n);
  if (Z_issquare(n)) pari_err_IMPL("square Pell solutions");
  pari_sp ltop = avma;
  GEN C, t, x, y;
  long k = 1;
  long myprec = 125;
...
{
  pari_sp ltop = avma;
  if (!is_matvec_t(typ(v))) pari_err_TYPE("checkmult", v);
  ...
  if (!isint1(gel(v, 1)))
  {
    if (verbose)
      pari_printf(
        "Only sequences starting with 1 are considered multiplicative.\n");
    return 0;
  }
  ...
  for (n = 6; n < l1; ++n)
  {
    if (uisprimepower(n, &dummy)) continue;
    f = factoru_pow(n);
    pr = gel(f, 1);
    ex = gel(f, 2);
    pe = gel(f, 3);
    ...
    if (!equalii(gel(v, n), target))
    {
      if (verbose)
      {
        pari_printf("Not multiplicative: a(%ld) = %Ps != %Ps = ", n, gel(v, n),
                    target);
        for (i = 1; i < l2; ++i)
        {
          if (i > 1) pari_printf(" * ");
...
{
  pari_sp ltop = avma;
  if (!is_matvec_t(typ(v))) pari_err_TYPE("checkcmult", v);
  ...
  for (n = 4; n < l1; ++n)
  {
    ...
    if (!equalii(gel(v, n), target))
    {
      if (verbose)
      {
        pari_printf("Not completely multiplicative: a(%ld) = %Ps != %Ps = ", n,
                    gel(v, n), target);
        for (i = 1; i < l2; ++i)
        {
          if (i > 1) pari_printf(" * ");
...
{
  pari_sp ltop = avma;
  if (!is_matvec_t(typ(v))) pari_err_TYPE("checkadd", v);
  ...
  {
    if (verbose)
      pari_printf("Only sequences starting with 0 are considered additive.\n");
    return 0;
  }
  ...
  for (n = 6; n < l1; ++n)
  {
    ...
    if (!gequal(gel(v, n), target))
    {
      if (verbose)
      {
        pari_printf("Not additive: a(%ld) = %Ps != %Ps = ", n, gel(v, n),
                    target);
        for (i = 1; i < l2; ++i)
        {
          if (i > 1) pari_printf(" + ");
...
{
  pari_sp ltop = avma;
  if (!is_matvec_t(typ(v))) pari_err_TYPE("checkcadd", v);
  ...
  // sensible.
  if (!gequalgs(gel(v, 1), 0)) return 0;
  ...
  for (n = 4; n < l1; ++n)
  {
    ...
    if (!equalii(gel(v, n), target))
    {
      if (verbose)
      {
        pari_printf("Not completely additive: a(%ld) = %Ps != %Ps = ", n,
                    gel(v, n), target);
        for (i = 1; i < l2; ++i)
        {
          if (i > 1) pari_printf(" + ");
...
{
  pari_sp ltop = avma;
  if (!is_matvec_t(typ(v))) pari_err_TYPE("checkdiv", v);
    ...
    if (signe(vn))
    {
      for (i = n + n; i < l1; i += n)
      {
        if (!dvdii(gel(v, i), vn))
        {
          if (verbose)
            pariprintf(
              "Not a divisibility sequence: a(%ld) = %Ps does not divide a(%ld) = %Ps.\n",
              n, vn, i, gel(v, i));
          avma = ltop;
          return 0;
        }
    ...
    else
    {
      for (i = n + n; i < l1; i += n)
      {
        if (signe(gel(v, i)))
        {
          if (verbose)
            pariprintf(
              "Not a divisibility sequence: a(%ld) = 0 does not divide a(%ld) = %Ps.\n",
              n, i, gel(v, i));
          avma = ltop;
          return 0;
        }
...
classno_fast(GEN n)
{
  if (lgefint(n) == 3) return classno(n);
...
{
  if (typ(n) != t_INT) pari_err_TYPE("HurwitzClassNumber", n);
  if (signe(n) < 1)
  {
    if (isintzero(n)) return gdiv(gen_m1, utoipos(12));
  ...
  for (i = 1; i < mx; ++i)
  {
    ...
    if ((1 << mod4(D)) & 0x9)
    {
      // mod4(D) = |D| mod 4, so this selects |D| in {0, 3} mod 4 thus D in {0,
      // 1} mod 4
      ret = gadd(ret, gdiv(classno_fast(D), gmaxsg(1, addis(D, 6))));
      ret =
        gerepileupto(btop, ret); // perhaps do this only when the stack is low?
    }
  }
  ret = gerepileupto(ltop, ret);
  return ret;
...
taup_small(ulong p)
{
  ...
  for (k = 1; k < p; ++k)
  {
    ...
    if (low_stack(st_lim, stack_lim(ltop, 1))) ret = gerepileuptoint(ltop, ret);
  }
  ret = diviuexact(subii(addii(mulsi(65, sumdivk(utoipos(p), 11)),
                               mulsi(691, sumdivk(utoipos(p), 5))),
                         mulsi(174132, ret)),
                   756);
  return gerepileuptoint(ltop, ret);
}
...
taup_big(GEN p)
{
  ...
  for (t = gen_1; cmpii(t, lim) <= 0; t = addis(t, 1))
  {
    ...
    ret = gadd(ret, tmp);
    if (low_stack(st_lim, stack_lim(btop, 1))) gerepileall(btop, 2, &t, &ret);
  }
  ret =
    gsub(gsubgs(gdivgs(mulii(powiu(p, 5), HurwitzClassNumber(p4)), 2), 1), ret);
  return gerepileupto(ltop, ret);
}
...
taup(GEN p, long e)
{
  ...
  for (j = 0; j <= l2; ++j)
  {
    ...
    GEN tmp = mulii(mulii(binomialuu(e - j, e - 2 * j), powiu(t, e - 2 * j)),
                    powiu(p, 11 * j));
    if (j & 1)
      ret = subii(ret, tmp);
    else
      ret = addii(ret, tmp);
    if (low_stack(st_lim, stack_lim(btop, 1))) ret = gerepileupto(btop, ret);
  }
  return gerepileupto(ltop, ret);
}
...
tau(GEN n)
{
  if (typ(n) != t_INT) pari_err_TYPE("tau", n);
  pari_sp ltop = avma;
