...
getBValue(char* line)
{
  ...
  while (line[start] == ' ' || line[start] == '\t')
    start++;
  if (line[start] == '#') return NULL; // Comment
  if (line[start] == '-') start++;
  ...
  if (start == end) return NULL; // Blank line, or no numbers found
  line[end] = '\0';
...
bfilein(char* name)
{
...
#else
    if (strlen(name) == 11 && name[0] == 'b' && name[1] >= '0' &&
        name[1] <= '9' && name[2] >= '0' && name[2] <= '9' && name[3] >= '0' &&
        name[3] <= '9' && name[4] >= '0' && name[4] <= '9' && name[5] >= '0' &&
        name[5] <= '9' && name[6] >= '0' && name[6] <= '9' && name[7] == '.' &&
        name[8] == 't' && name[9] == 'x' && name[10] == 't')
    {
      char command[65];
      sprintf(command, "wget http://oeis.org/A%c%c%c%c%c%c/%s", name[1],
              name[2], name[3], name[4], name[5], name[6], name);
      int result = system(command);
      if (result == -1) pari_warn(warner, "Download failed.");
    }
    f = fopen(name, "r");
    if (!f) pari_err_FILE("input file", name);
#endif
  }
  ...
  while (fgets(line, MAX_LINELEN, f) != NULL)
  {
    if (strlen(line) > MAX_LINELEN - 5)
      pari_err(e_MISC,
               "Maximum line length exceeded; b-file probably not valid");
    ...
    if (++i > MAX_VECLEN)
    {
      pari_warn(warner, "only %d terms used; b-file has unread terms",
                MAX_VECLEN);
      break;
    }
...
bfileout(char* filename, GEN name, GEN v, GEN Anum, long offset)
{
  ...
  if (f)
  {
    pari_warn(warner, "File `%Ps' already exists. Moving to bfile.old...",
              name);
    fclose(f);
  ...
  {
    GEN e = gel(v, i);
    if (typ(e) != t_INT) pari_err_TYPE("bfile", e);
...
fnice(GEN n)
{
  ...
  GEN f, s, s1, p1;
  long l2;
  if (typ(n) != t_INT) pari_err_TYPE("fnice", n);
  if (signe(n) < 0)
  {
    s = strtoGENstr("-");
    n = negi(n);
  ...
  for (i = 2; i <= l2; ++i)
  {
    ...
    s = Str(mkvec2(s, s1));
    if (low_stack(st_lim, stack_lim(btop, 1))) gerepileall(btop, 2, &s1, &s);
  }
...
tonice(GEN o, long prec)
{
  ...
  if (typ(o) == t_POL)
  {
    ...
    if (gequal1(denom(t)))
    {
      ...
      o = simplify(o);
      if (typ(o) != t_INT) pari_err_IMPL("multivariate polynomials in nice");
      o = polcoeff0(o, 0, -1);
      if (gcmpgs(o, 0) > 0) s = Str(mkvec3(s, strtoGENstr(" + "), o));
      if (gcmpgs(o, 0) < 0) s = Str(mkvec3(s, strtoGENstr(" - "), gneg(o)));
      s = gerepileupto(ltop, s);
      return s;
    }
    ...
    p3 = Str(
      mkvecn(5, numer(t), strtoGENstr("("), o, strtoGENstr(")/"), denom(t)));
